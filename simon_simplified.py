...
MOTION_DIR = SCRIPT_DIR / "simon_task" / "MotionLibrary"
CORE_POOL_PATH = MOTION_DIR / "core_pool.npy"
...
def run_calibration_trial(win, mouse, prop, angle_bias, show_distractor=True):
    ...
    square_pos = np.array([-OFFSET_X, 0.0], dtype=float)
    dot_pos = np.array([OFFSET_X, 0.0], dtype=float)
...
def run_control_only_phase(win, mouse, prop, label, targets_per_phase=3, max_target_time=20.0):
    ...
    rect_bounds = get_control_rect(dot_radius)
    left, right, bottom, top = rect_bounds
    rect_shape = visual.Rect(
        win,
        width=right - left,
        height=top - bottom,
        lineColor="white",
        lineWidth=2,
        fillColor=None,
        pos=((left + right) / 2, (bottom + top) / 2),
    )
    target_x = visual.TextStim(win, "X", color="yellow", height=32)
    reached_msg = visual.TextStim(win, "Target reached!", color="lime", height=30, pos=(0, -200))

    def random_target():
        margin = 40
        tx = rng.uniform(left + margin, right - margin)
        ty = rng.uniform(bottom + margin, top - margin)
        return np.array([tx, ty], dtype=float)

    for target_idx in range(targets_per_phase):
        target_pos = random_target()
        target_x.pos = target_pos
        ...
        while not reached and clock.getTime() < max_target_time:
            ...
            if inside_rect(proposed_pos, rect_bounds):
                position = proposed_pos
            else:
                position = clamp_to_rect(proposed_pos, rect_bounds)
...
def main():
    ...
    show_message(
        win,
        "Three-Part Task:\n\n"
        "1) Calibration with two shapes.\n"
        "2) Easy condition (only the controlled shape).\n"
        "3) Hard condition (only the controlled shape).\n\n"
    ...
    run_control_only_phase(win, mouse, easy_prop, "Easy")

    show_message(win, "Ready for the harder condition?\nPress SPACE to continue.")
    run_control_only_phase(win, mouse, hard_prop, "Hard")
